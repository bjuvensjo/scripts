...
def create_patch(repo, since_tag, tag, output):
    ...
    makedirs(patch_dir)
    cmd = 'git log -p --reverse --pretty=email --full-index --binary --stat -m --first-parent {}{} > {}/tag.patch'.format(
        since_tag + '..' if since_tag else '', tag, patch_dir)
...
def apply_patch(repo, tag, patch_dir):
    ...
    return [run_command(cmd, True, repo) for cmd in ['git apply {}/*'.format(patch_dir),
                                                     'git add --all',
                                                     'git commit -m {}'.format(tag),
                                                     'git tag -a {} -m {}'.format(tag, tag)]]


def get_tags(repo, tag_pattern):
    return [tag for tag in run_command('git tag', True, repo)[1].split('\n') if match(r'{}'.format(tag_pattern), tag)]
...
def is_valid(patchs_tags, applied_tags):
    return all([p == a for p, a in zip(patchs_tags, applied_tags)]) and not len(applied_tags) > len(patchs_tags)
...
def main(patch_repo, tag_pattern, output, apply_repo):
    ...
    if is_valid(patchs_tags, applied_tags):
        ...
        created_patches = [create_patch(patch_repo, since_tag, until_tag, output)
                           for since_tag, until_tag in patch_ranges]
        applied_patches = [apply_patch(apply_repo, tag, patch_dir) for tag, patch_dir, out in created_patches]
...
def parse_args(args):
    parser = ArgumentParser(description='Create patches of tags and applies, commits and tags them in another repo.')
    parser.add_argument('tag_pattern', help='A tag pattern.')
    parser.add_argument('apply_repo', help='The repo to apply patches to.')
    parser.add_argument('-p', '--patch_repo', help='The repo to patch from.', default='.')
    parser.add_argument('-o', '--output', help='A directory to put patches in.', default='./patch')
    return parser.parse_args(args)


if __name__ == '__main__':
    args = parse_args(argv[1:])
    main(args.patch_repo, args.tag_pattern, args.output, args.apply_repo)
