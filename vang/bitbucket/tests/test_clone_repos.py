...
def test_should_be_cloned(mock_is_included):
    ...
    assert [call("repo", ["excludes"], ["includes"])] == mock_is_included.mock_calls
...
def test_get_projects_commands(mock_get_clone_urls):
    ...
    assert [call("projects", True, "branch", False)] == mock_get_clone_urls.mock_calls
...
def test_get_repos_commands(mock_get_clone_urls, mock_print):
    ...
    assert [call({"project"}, True, "branch", False)] == mock_get_clone_urls.mock_calls
    assert [
...
def test_get_config_commands(
    mock_has_branch, mock_get_clone_urls, mock_should_be_cloned
):
    assert [
        ("clone_dir", "project", "repo1", "command"),
        ("clone_dir", "project", "repo2", "command"),
