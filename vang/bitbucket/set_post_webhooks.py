...
def delete_post_web_hook(spec, hook_id):
    return call(f'{get_uri(spec)}/{hook_id}', only_response_code=True, method='DELETE')


def create_post_web_hook(spec, hook):
    return call(get_uri(spec), only_response_code=True, method='POST', request_data=hook)


def get_post_web_hooks(spec):
    return call(get_uri(spec))


def set_repo_post_web_hooks(spec, hooks):
    try:
        # Delete existing post web hooks
        for hook in get_post_web_hooks(spec):
            delete_response_code = delete_post_web_hook(spec, hook['id'])
            if delete_response_code != 204:
                return spec, False, f'Can not delete {hook["id"]}: {delete_response_code}'

        # Create post web hooks
        for hook in hooks:
            create_response_code = create_post_web_hook(spec, hook)
            if create_response_code != 200:
                return spec, False, f'Can not create {hook}: {create_response_code}'

        return spec, True, f'Set post webhooks {hooks}'
    except IOError as ioe:
        return spec, False, ioe


def set_post_web_hooks(specs, hooks, max_processes=10):
    with Pool(processes=max_processes) as pool:
        return pool.starmap(set_repo_post_web_hooks, product(specs, (hooks,)))
...
def main(title, url, dirs, repos=None, projects=None):
    ...
    hooks = (hook,)
    for spec, result, message in set_post_web_hooks(specs, hooks):
        if result:
...
def parse_args(args):
    parser = argparse.ArgumentParser(description='Set Bitbucket post webhooks. '
                                                 'The hooks are based on a template defined in this script.')
    parser.add_argument('title', help='The post webhook title')
    parser.add_argument('url', help='The post webhook url')
