...
def list_build_definitions(organisations=None, projects=None, filter_name=None):
    ...
    build_definitions = {
        build_definition["name"]: build_definition
        for project in projects
        for build_definition in call(
            f"/{project}/_apis/build/definitions?api-version=3.2{query}"
        )["value"]
    }
...
def parse_args(args):
    ...
    optional_group.add_argument(
        "-i", "--ids", action="store_true", help="Get only build definition ids"
    )
    optional_group.add_argument(
        "-n", "--names", action="store_true", help="Get only build definition names"
    )
    optional_group.add_argument(
        "-ni",
        "--names_and_ids",
    ...
    optional_group.add_argument(
        "-u", "--urls", action="store_true", help="Get only build definition urls"
    )
    optional_group.add_argument(
        "-w",
        "--web_urls",
