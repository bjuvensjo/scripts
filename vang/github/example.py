...
def query_github(endpoint, token, query, variables={}, operation_name=None):
    json = {
        'query': query
    }
    if variables:
...
def get_login(endpoint, token):
    ...
    return query_github(endpoint, token, query)['data']['viewer']['login']


Repository = namedtuple('Repository', 'name branches repo')
...
def get_repositories(endpoint, token, login, first=100, with_message=True):
    ...
    response = query_github(endpoint, token, query, variables)
...
def main(endpoint: str, token: str, login: str, first: int, with_message: bool) -> None:  # pragma: no cover
    l = login or get_login(endpoint, token)
    repositories = get_repositories(endpoint, token, l, first, with_message)
