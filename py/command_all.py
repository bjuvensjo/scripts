...
def get_work_dirs(find, root):
    return [
        Path(realpath(p)).parent
        for p in glob(normpath('{}/**/{}'.format(root, find)), recursive=True)
    ]
...
def execute_in_parallel(root, commands, find='.git/'):
    commands_and_work_dirs = tuple(
        product(get_command(commands), get_work_dirs(find, root)))
...
def parse_args(args):
    ...
    parser.add_argument(
        ...
        '--find',
        help=
        'The file/dir to be in the directory in which to execute, default ".git/"',
        default='.git/')
    parser.add_argument(
        '-s',
        '--sequence',
